...
//	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave it be.
	...
	if(std::isfinite(idepth_max)) {
        ...
        // 得到在图像上的相对于(uMin, vMin)平移方向，然后根据magic number确定uMax, vMax
		uMax = uMin + dist*dx*d;
		vMax = vMin + dist*dy*d;
...
ImmaturePointStatus ImmaturePoint::traceStereo(FrameHessian* frame, CalibHessian* HCalib, bool mode_right)
{
	...
	K(0,0) = HCalib->fxl(); K(0,1) = 0;             K(0,2) = HCalib->cxl();
    K(1,0) = 0;             K(1,1) = HCalib->fyl(); K(1,2) = HCalib->cyl();
	...
	float vMax;
	Vec3f ptpMax;
    //  maxPixSearch < 對角線長度 × 系數 < (長+寬）x 系數
	float maxPixSearch = (wG[0]+hG[0])*setting_maxPixSearch;
	...
	for(int it=0;it<setting_trace_GNIterations;it++) {
		...
		for(int idx=0;idx<patternNum;idx++) {
            ...
            // 残差与距离的导数H， 乘上step，即delta距离，即b残差增量
			H += hw*dResdDist*dResdDist;   //  d{残差}/d{距离}
			b += hw*residual*dResdDist;
		...
		float idepth)
{
	...
	for(int idx=0;idx<patternNum;idx++) {
        ...
        // energy应该是残差的加权平方和，这里另外有一个huber考量，
        // 当残差绝对值大于huber阈值的时候，将hw设为一个小于1的值
        // hw意思是huber weight
